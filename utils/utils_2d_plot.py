...
def get_2d_pred_GWI(model, dataset):
    ...
    inputs_flattened, _, _ = gen_grid_inputs(POINTS_PER_AXIS,
                                             X1_RANGE, X2_RANGE)
    mean, std = model.pred_mean_std(torch.from_numpy(inputs_flattened).float().to(model.device))
    contour_std = std.reshape(POINTS_PER_AXIS, POINTS_PER_AXIS)

    slice_points, _, _ = get_slice_points(dataset)
    mean, std = model.pred_mean_std(torch.from_numpy(slice_points).float().to(model.device))
    slice_mean = mean.squeeze()#[:, 0]
    slice_std = std.squeeze()#[:, 0]
    return contour_std.cpu().numpy(), slice_mean.cpu().numpy(), slice_std.cpu().numpy()
...
def make_contour_plot(fig, ax, std, X, dataset):
    ...
    _, x1_grid, x2_grid = gen_grid_inputs(POINTS_PER_AXIS,
                                                         X1_RANGE, X2_RANGE)
    cnt = ax.contourf(x1_grid, x2_grid, std, levels=200,cmap=plt.get_cmap('viridis'))
